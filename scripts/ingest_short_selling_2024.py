...
"""
Script: Ingest Short Selling History (2024)
Target: 2024-01-01 ~ 2024-12-31
TR: opt10014 (공매도추이요청)
"""
import sys
import os
...
SHORT_DIR = DATA_DIR / "history" / "short_selling" / "2024"
...
class ShortSellingIngester:
    ...
    def process_next(self):
        ...
        print(f"[{self.current_idx+1}/{len(self.targets)}] Requesting Short Selling Data (2024) for {symbol}...")
        
        self.ocx.dynamicCall("SetInputValue(QString, QString)", "종목코드", symbol)
        self.ocx.dynamicCall("SetInputValue(QString, QString)", "시간구분", "1")
        self.ocx.dynamicCall("SetInputValue(QString, QString)", "시작일자", "20240101")
        self.ocx.dynamicCall("SetInputValue(QString, QString)", "종료일자", "20241231")
    ...
    def on_receive_tr_data(self, screen_no, rq_name, tr_code, record_name, prev_next, data_len, err_code, msg, splm_msg):
        if tr_code == "opt10014":
            ...
            self.summary_rows.append({
                "symbol": symbol,
                "total_short_vol_2024": total_short_vol,
                "total_short_val_2024": total_short_val,
    ...
    def finish(self):
        ...
        if self.summary_rows:
            ...
            map_path = DATA_DIR / "short_selling_map_2024.csv"
