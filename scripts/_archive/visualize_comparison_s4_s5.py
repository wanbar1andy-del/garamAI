...
def main():
    ...
    # Load S5 (Intraday Stop)
    path_s5 = results_dir / "equity.csv"
    if not path_s5.exists():
        print(f"Error: S5 results not found at {path_s5}")
        return
    df_s5 = pd.read_csv(path_s5)
    df_s5['date'] = pd.to_datetime(df_s5['date'])
    df_s5.set_index('date', inplace=True)
    
    # Calculate Metrics
    ret_s4 = (df_s4['equity'].iloc[-1] / df_s4['equity'].iloc[0] - 1) * 100
    ret_s5 = (df_s5['equity'].iloc[-1] / df_s5['equity'].iloc[0] - 1) * 100
    
    mdd_s4 = ((df_s4['equity'] - df_s4['equity'].cummax()) / df_s4['equity'].cummax()).min() * 100
    mdd_s5 = ((df_s5['equity'] - df_s5['equity'].cummax()) / df_s5['equity'].cummax()).min() * 100
    
    print(f"S4 (Standard): Return {ret_s4:.2f}%, MDD {mdd_s4:.2f}%")
    print(f"S5 (Intraday Stop): Return {ret_s5:.2f}%, MDD {mdd_s5:.2f}%")

    # Plot
    fig, ax = plt.subplots(figsize=(12, 6))
    
    ax.plot(df_s4.index, df_s4['equity'], color='#95a5a6', linewidth=2, label=f'S4 (Standard): +{ret_s4:.2f}% (MDD {mdd_s4:.2f}%)')
    ax.plot(df_s5.index, df_s5['equity'], color='#e74c3c', linewidth=2, label=f'S5 (Intraday Stop): +{ret_s5:.2f}% (MDD {mdd_s5:.2f}%)')
    
    ax.set_title('Impact of Intraday Stop-Loss (Exit at Close if Loss)', fontsize=14, fontweight='bold')
    ...
    output_path = "c:/garam/garam/comparison_s4_vs_s5.png"
