...
def simulate_turbo_v2_1(df):
    """
    Turbo Overlay V2.1 시뮬레이션
    
    V2.1 개선:
    1. Exit: daily_return <= -2% 시 즉시 Turbo OFF (기존 -3%)
    2. Exit: 3일 합 <= -3% 시 Turbo OFF (기존 -4%)
    3. 재진입 조건: V2와 동일
    """
    df = df.copy()
    
    df['turbo_active_v2_1'] = False
    df['concentration'] = 0.0
    df['turbo_multiplier_v2_1'] = 1.0
    df['turbo_equity_v2_1'] = df['equity'].copy()
    df['exit_reason_v2_1'] = ''
    df['reentry_reason_v2_1'] = ''
    ...
    for idx in range(len(df)):
        ...
        if is_turbo_active:
            ...
            # Exit 조건 1: 단일일 급락 -2% (V2: -3%)
            if multiplier >= 2.0 and row['daily_return'] <= -0.02:
                is_turbo_active = False
                turbo_off_date = date
                turbo_off_equity = equity
                turbo_off_dd = (equity - portfolio_high) / portfolio_high if portfolio_high > 0 else 0
                df.loc[df.index[idx], 'exit_reason_v2_1'] = f'Daily drop {row["daily_return"]*100:.1f}%'
                multiplier = 1.0
            
            # Exit 조건 2: 연속 하락 (3일 합 <= -3%) (V2: -4%)
                ...
                if sum_3d <= -0.03:
                    ...
                    turbo_off_equity = equity
                    turbo_off_dd = (equity - portfolio_high) / portfolio_high if portfolio_high > 0 else 0
                    df.loc[df.index[idx], 'exit_reason_v2_1'] = f'3-day decline {sum_3d*100:.1f}%'
                    multiplier = 1.0
            ...
            if turbo_off_date is not None:
                ...
                if days_since_exit >= 3:
                    ...
                    if recovery_ratio >= 0.5 and momentum_ok and concentration_ok:
                        ...
                        df.loc[df.index[idx], 'reentry_reason_v2_1'] = f'Recovery {recovery_ratio*100:.0f}%, Mom {recent_sum*100:.1f}%'
            else:
                if concentration >= 0.8:
                    is_turbo_active = True
                    df.loc[df.index[idx], 'reentry_reason_v2_1'] = 'Initial entry'
        ...
        df.loc[df.index[idx], 'turbo_active_v2_1'] = is_turbo_active
        df.loc[df.index[idx], 'turbo_multiplier_v2_1'] = multiplier
        ...
        df.loc[df.index[idx], 'turbo_equity_v2_1'] = equity
    
    return df

if __name__ == "__main__":
    print("V2.1 시뮬레이션 모듈 로드 완료")
