...
class IntradayHeroStrategyDefenseV2(bt.Strategy):
    """
    Never Stuck Defense Patch V2 (Option A+)
    - Time Stop: 100m Hard, 60m Soft (MFE < 0.7%)
    - Consecutive Loss: 5x
    - Daily Limit: -1.5%
    - Cooldown: 10m (only after loss)
    """
    params = (
        ("warmup", 30),
    ...
    def __init__(self):
        ...
        # Per Symbol State
        self.entry_times = {} # d -> datetime
        self.last_exit_time = {} # d -> datetime
        self.last_exit_pnl = {} # d -> pnl
        self.highest_price_in_trade = {} # d -> price (for MFE)
    ...
    def next(self):
        ...
        for d in self.datas:
            ...
            if last_exit and last_pnl < 0: # Only if loss
            ...
            if d.volume[0] > prev_vol * self.p.vol_mult and d.close[0] >= prev_high:
                ...
                if size > 0:
                    ...
                    self.highest_price_in_trade[d] = d.close[0] # Init MFE
...
def run_defense_replay():
    ...
    for date_str in dates:
        print(f"\n--- Defense Sim {date_str} (Start: {current_cash:,.0f}) ---")
        ...
        cerebro.addstrategy(IntradayHeroStrategyDefenseV2)
        strat = cerebro.run()[0]
    ...
    pd.DataFrame(equity_curve).to_csv("GARAM_Data/5day_equity_defense.csv", index=False)
    print("Saved GARAM_Data/5day_equity_defense.csv")
